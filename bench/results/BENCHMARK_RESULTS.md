# RustyCSV Benchmark Results

**Date:** January 25, 2026
**RustyCSV Version:** 0.2.0
...
RustyCSV provides significant speedups for throughput-bound workloads:

| Metric | RustyCSV (best) | NimbleCSV | Improvement |
|--------|-----------------|-----------|-------------|
...
- **Rust NIF** allocation is separate from BEAM
- **Total retained** includes both heap and binary references

### Rust NIF Memory (peak allocation during parsing)
...
**Bottom line:** Both parsers use memory proportional to the data. RustyCSV splits between Rust and BEAM; NimbleCSV is entirely on BEAM. Neither is dramatically more efficient.
...
**Key insight:** RustyCSV can process arbitrary binary chunks. NimbleCSV requires line-delimited input. This is a **capability difference**, not a speed difference.

## Correctness

- All RustyCSV strategies produce identical output ✅
- RustyCSV output matches NimbleCSV ✅
...
4. **Memory usage is comparable** - RustyCSV allocates on Rust side, NimbleCSV on BEAM. Neither dramatically more efficient.

5. **BEAM reductions are minimal** - Up to 14,228x fewer reductions, reducing scheduler load

6. **Streaming is a capability difference** - RustyCSV handles binary chunks; speed is comparable for line-based

7. **Quoted fields show largest gains** - 17.9x faster due to efficient escape handling in Rust
